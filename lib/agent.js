...
async function sync() {
  ...
  try {
    ...
    if (upcoming.status === 'fulfilled') {
      ...
      // New format: { outcomes: [...], _meta: {...} }
      if (Array.isArray(u.outcomes)) {
        state.upcomingCampaigns = u.outcomes;
      } else {
        // Legacy format: per-product object
        const KNOWN_PRODUCTS = Object.keys(PRODUCT_MAP);
        state.upcomingCampaigns = Object.entries(u)
          .filter(([product]) => KNOWN_PRODUCTS.includes(product))
    ...
    if (outcomePreviews.status === 'fulfilled') {
      const KNOWN_PRODUCTS = Object.keys(PRODUCT_MAP);
      const raw = outcomePreviews.value;
      state.outcomePreviewsData = Object.fromEntries(
        Object.entries(raw).filter(([k]) => KNOWN_PRODUCTS.includes(k))
      );
    }
    ...
    for (const [product, deviceMap] of Object.entries(PRODUCT_MAP)) {
      ...
      for (const [device, cacheId] of Object.entries(deviceMap)) {
        ...
        const isArmed = banner.status === 'armed';
        const statusTag = isArmed ? ' (armed)' : '';
        const triggerNote = isArmed && banner.scheduledAt
        ...
        await store.update(cacheId, {
          ...
          description: `[CIA] ${banner.campaignName} — ${banner.outcome}${statusTag}`,
          note: `Auto-updated by Campaign Image Agent. Campaign: "${banner.campaignName}", Outcome: ${banner.outcome}${statusTag}${triggerNote}`,
